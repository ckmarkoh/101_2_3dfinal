...
/// function pointers
static void (*renderFunc)(const LCamera cam, const LLight lit, FrameBuffer * colorBuff, RENDER_MODE rmode, PROJECT_MODE pmode, int modelIndex) = NULL;
...
PROJECT_MODE projectMode = PERSPECTIVE;
int modelIndex = 0;
...
void showHelp()
{
	...
	printf("\tW/w: increase the model index.\n");
	printf("\tE/e: decrease the model index.\n");
	...
	printf("\tP/p: switch projecting mode.\n");
...
void drawFunc()
{
	...
	{
		(*renderFunc)(cam, lit, colorBuff, renderMode, projectMode, modelIndex);
...
void idle()
{
	...
	{
		LViewer::flag['M'-'A'] ^=1;
		renderMode = (RENDER_MODE) (((int)renderMode +1) % (int)RENDER_MODE::NUM);
	...
	{
		LViewer::flag['P'-'A'] ^=1;
		projectMode = (PROJECT_MODE) (((int)projectMode +1) % 2);
		printf("Current render Mode is %s\n", (projectMode==0)? "PERSPECTIVE":"PARALLEL");
	}
	...
	if(!LViewer::flag['W'-'A'])
	{
		LViewer::flag['W'-'A'] ^=1;
		modelIndex++;
		printf("Current modelIndex is %d\n", modelIndex);
	}
...
int initAndRunLViewer( const int winW, const int winH, void (*render)(const LCamera cam, const LLight lit, FrameBuffer * colorB, RENDER_MODE rmode, PROJECT_MODE pmode, int modelIndex), void (*initFun)())
{
	renderFunc = render;
	initFunc   = initFun;
