...
namespace LLib
{
	namespace Viewer
	{
		...
		/* =====================================================*/
			LTranslateMatrix LViewer::viewMatrix(vec3( 0, 500, 0), vec3( 0, 0, 0), vec3(1,0,0));
		...
		// class static initialize
			...
			vec3 LViewer::viewerPosition		= vec3( 0, 500, 0);		// vec3( 0, 0, -50);
			...
			double	LViewer::nearPlane		= 250.0;
		...
		void _keyboard(unsigned char key, int x, int y)
		{
			...
			{
				if( Viewer::FINISH == (*LViewer::m_keyboard)(key, x, y))
					return ;
			}
...
// numbers [0-9]
		...
		void _motion(int x, int y)
		{
			...
			{
				if( Viewer::FINISH == (*LViewer::m_motionFun)( x, y))
					return ;
			}
		...
		// user defined keyboard function
			...
			{
				if( Viewer::FINISH == (*LViewer::m_mouseFun)(button, state, x, y))
					return ;
			}

			if (LViewer::rotating == true) cout << "rotating" << endl;
		//	double difX, difY;
